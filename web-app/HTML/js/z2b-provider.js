...
function loadProviderUXFull ()
{
    let toLoad = 'providerFull.html';
    if (buyers.length === 0)
    { $.when($.get(toLoad), deferredMemberLoad()).done(function (page, res)
      {setupProviderFull(page[0]);});
    }
    else{
        $.when($.get(toLoad)).done(function (page)
        {setupProviderFull(page);});
    }
}
...
function formatProviderOrders(_target, _orders)
{
    ...
    {(function(_idx, _arr)
        ...
        _str += '<table class="wide"><tr><th>'+textPrompts.orderProcess.orderno+'</th><th>'+textPrompts.orderProcess.status+'</th><th class="right">'+textPrompts.orderProcess.total+'</th><th colspan="3" class="right message">'+textPrompts.orderProcess.buyer+findMember(_arr[_idx].buyerId,buyers).companyName+'</th></tr>';
        _str += '<tr><th id ="p_order'+_idx+'" width="20%">'+_arr[_idx].orderNumber+'</th><th width="50%" id="p_status'+_idx+'">'+JSON.parse(_arr[_idx].status).text+': '+_date+'</th><th class="right">$'+_arr[_idx].amount+'.00</th>'+_action+'<br/><select id="shippers'+_idx+'">'+sh_string+b_string+'</th>'+_button+'</tr></table>';
