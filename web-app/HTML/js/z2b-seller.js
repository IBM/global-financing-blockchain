...
function loadSellerUXFull ()
{
    let toLoad = 'seller.html';
    if (buyers.length === 0) 
    { $.when($.get(toLoad), deferredMemberLoad()).done(function (page, res)
    {setupSellerFull(page[0]);});
    }
    else{
        $.when($.get(toLoad)).done(function (page)
        {setupSellerFull(page);});
    }
}
...
function setupSellerFull(page, port)
{
    $('#body').empty();
    $('#body').append(page);
    if (s_alerts.length == 0) 
...
function formatSellerOrders(_target, _orders)
{
    ...
    {(function(_idx, _arr)
        ...
        _str += '<tr><th id ="s_order'+_idx+'" width="20%">'+_arr[_idx].orderNumber+'</th><th width="50%" id="s_status'+_idx+'">'+JSON.parse(_arr[_idx].status).text+'</th><th class="right">$'+_arr[_idx].amount+'.00</th>'+_action+'<br/><select id="providers'+_idx+'">'+p_string+'</th>'+_button+'</tr></table>';
