...
function loadShipperUXFull ()
{
    let toLoad = 'shipperFull.html';
    if (buyers.length === 0)
    { $.when($.get(toLoad), deferredMemberLoad()).done(function (page, res)
    {setupShipperFull(page[0]);});
    }
    else{
        $.when($.get(toLoad)).done(function (page)
        {setupShipperFull(page);});
    }
}
...
function formatShipperOrders(_target, _orders)
{
    ...
    {(function(_idx, _arr)
        ...
        _str += '<table class="wide"><tr><th>'+textPrompts.orderProcess.orderno+'</th><th>'+textPrompts.orderProcess.status+'</th><th class="right">'+textPrompts.orderProcess.total+'</th><th colspan="3" class="right message">Buyer: '+findMember(_arr[_idx].buyerId,buyers).companyName+'</th></tr>';
        _str += '<tr><th id ="sh_order'+_idx+'" width="20%">'+_arr[_idx].orderNumber+'</th><th width="50%" id="sh_status'+_idx+'">'+JSON.parse(_arr[_idx].status).text+': '+_date+'</th><th class="right">$'+_arr[_idx].amount+'.00</th>'+_action+_statusText+'</th>'+_button+'</tr></table>';
