...
{  let toLoad = 'createOrder.html';
    ...
    showLoad();
    $.when($.get(toLoad), $.get('/fabric/client/getItemTable')).done(function (page, _items)
    {
        hideLoad();
        ...
        $('#submitNewOrder').on('click', function ()
            ...
            showLoad();
            $.when($.post('/fabric/client/addOrder', options)).done(function(_res) {
                hideLoad();    
...
function formatOrders(_target, _orders)
{
    ...
    {(function(_idx, _arr)
      {let _action = '<th><select id=b_action'+_idx+'><option value="'+textPrompts.orderProcess.NoAction.select+'">'+textPrompts.orderProcess.NoAction.message+'</option>';
        ...
        _str += '<table class="wide"><tr><th>'+textPrompts.orderProcess.orderno+'</th><th>'+textPrompts.orderProcess.status+'</th><th class="right">'+textPrompts.orderProcess.total+'</th><th colspan="3" class="right message">'+textPrompts.orderProcess.seller+findMember(_arr[_idx].sellerId,sellers).companyName+'</th></tr>';
        _str += '<tr><th id ="b_order'+_idx+'" width="20%">'+_arr[_idx].orderNumber+'</th><th width="50%" id="b_status'+_idx+'">'+JSON.parse(_arr[_idx].status).text+'</th><th class="right">$'+_arr[_idx].amount+'.00</th>'+_action+r_string+_button+'</tr></table>';
