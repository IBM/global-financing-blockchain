...
function listFinanceOrders()
{
    ...
    showLoad();
    $.when($.post('/fabric/client/getMyOrders', options)).done(function(_results)
    {
        hideLoad();
...
function formatFinanceOrders(_target, _orders)
{
    ...
    {(function(_idx, _arr)
        ...
        _str += '<tr><th id ="f_order'+_idx+'" class="showFocus" width="20%">'+_arr[_idx].orderNumber+'</th><th width="50%" id="f_status'+_idx+'">'+JSON.parse(_arr[_idx].status).text+': '+_date+'</th><th class="right">$'+_arr[_idx].amount+'.00</th>'+_action+'</th>'+_button+'</tr></table></div>';
    ...
    for (let each in _orders)
        {(function(_idx, _arr)
        {
            ...
            $('#f_btn_'+_idx).on('click', function ()
            {
                ...
                showLoad();
                $.when($.post('/fabric/client/orderAction', options)).done(function (_results)
                { 
                    hideLoad();
...
function formatDetail(_cur, _order)
{
    ...
    _out += '<table class="wide"><tr><th id="action">'+textPrompts.financeCoOrder.status+'</th><th id="by">'+textPrompts.financeCoOrder.by+'</th><th id="comments">'+textPrompts.financeCoOrder.comments+'</th></tr>';
    _out += '<tr><td id="created">Created</td><td>'+_order.buyerId+'</td><td></td></tr>';
    _out += (_order.cancelled === '') ?  '<tr><td id="cancelled">'+textPrompts.financeCoOrder.cancelled+'?</td><td></td><td id="notCancel">'+textPrompts.financeCoOrder.notCancel+'</td><td></td></tr>' : '<tr><td id="cancelled">'+textPrompts.financeCoOrder.cancelled+'</td><td>'+_order.buyerId+'</td><td></td></tr>';
    _out += (_order.bought === '') ?  '<tr><td id="purchased">'+textPrompts.financeCoOrder.purchased+'</td><td></td><td id="noPurchase">'+textPrompts.financeCoOrder.noPurchase+'</td><td></td></tr>' : '<tr><td id="purchased">'+textPrompts.financeCoOrder.purchased+'</td><td>'+_order.buyerId+'</td><td></td></tr>';
    _out += (_order.ordered === '') ?  '<tr><td id="thirdParty">'+textPrompts.financeCoOrder.thirdParty+'</td><td></td><td id="nothirdParty">'+textPrompts.financeCoOrder.nothirdParty+'</td><td></td></tr>' : '<tr><td id="thirdParty">'+textPrompts.financeCoOrder.thirdParty+'</td><td>'+_order.sellerId+'</td><td></td></tr>';
    _out += (_order.dateBackordered === '') ?  '<tr><td id="backordered">'+textPrompts.financeCoOrder.backordered+'?</td><td></td><td id="notBackordered">'+textPrompts.financeCoOrder.notBackordered+'</td><td></td></tr>' : '<tr><td id="backordered">'+textPrompts.financeCoOrder.backordered+'</td><td>'+_order.providerId+'</td><td>'+_order.backOrder+'</td></tr>';
    _out += (_order.requestShipment === '') ?  '<tr><td id="shippingRequested">'+textPrompts.financeCoOrder.shippingRequested+'</td><td></td><td id="noRequestShip">'+textPrompts.financeCoOrder.noRequestShip+'</td><td></td></tr>' : '<tr><td id="shippingRequested">'+textPrompts.financeCoOrder.shippingRequested+'</td><td>'+_order.sellerId+'</td><td></td></tr>';
    _out += (_order.delivering === '') ?  '<tr><td id="shippingStarted">'+textPrompts.financeCoOrder.shippingRequested+'</td><td></td><td id="noDeliveryStart">'+textPrompts.financeCoOrder.noDeliveryStart+'</td><td></td></tr>' : '<tr><td id="shippingStarted">'+textPrompts.financeCoOrder.shippingStarted+'</td><td>'+_order.shipperId+'</td><td></td></tr>';
    _out += (_order.delivered === '') ?  '<tr><td id="delivered">'+textPrompts.financeCoOrder.delivered+'</td><td></td><td id="notDelivered">'+textPrompts.financeCoOrder.notDelivered+'</td><td></td></tr>' : '<tr><td id="delivered">'+textPrompts.financeCoOrder.delivered+'</td><td>'+_order.shipperId+'</td><td></td></tr>';
    _out += (_order.paymentRequested === '') ?  '<tr><td id="payRequested">'+textPrompts.financeCoOrder.payRequested+'</td><td></td><td id="noRequest">'+textPrompts.financeCoOrder.noRequest+'</td><td></td></tr>' : '<tr><td id="payRequested">'+textPrompts.financeCoOrder.payRequested+'</td><td></td><td></td></tr>';
    _out += (_order.disputeOpened === '') ?  '<tr><td id="disputed">'+textPrompts.financeCoOrder.disputed+'</td><td></td><td id="noDispute">'+textPrompts.financeCoOrder.noDispute+'</td><td></td></tr>' : '<tr><td id="disputed">'+textPrompts.financeCoOrder.disputed+'</td><td>'+_order.buyerId+'</td><td>'+_order.dispute+'</td></tr>';
    ...
    {_out +='<tr><td>Dispute Resolved</td><td></td><td>'+_order.resolve+'</td></tr>';}
    _out += (_order.orderRefunded === '') ?  '<tr><td>Refund?</td><td></td><td>(No Refund in Process)</td><td></td></tr>' : '<tr><td>Refund?</td><td></td><td>'+_order.refund+'</td></tr>';
    _out += (_order.approved === '') ?  '<tr><td>Payment Approved</td><td></td><td>(No Approval from Buyer)</td><td></td></tr>' : '<tr><td>Payment Approved</td><td>'+_order.buyerId+'</td><td></td></tr>';
    _out += (_order.paid === '') ?  '<tr><td>Paid</td><td></td><td>(UnPaid)</td><td></td></tr></table></div>' : '<tr><td>Paid</td><td>'+_order.financeCoId+'</td><td></td></tr></table></div>';
    return _out;
}
